...
class Config:
    ...
    # ==============================
    # 3.1 FRAMA 参数
    FRAMA_WINDOW = 100               # 计算周期 n_total
    FRAMA_N_SLOW = 100               # 慢速衰减周期
    
    # FRAMA 交易阈值
    THRESHOLD_STRONG_TREND = 1.25     # 强趋势阈值 D < 1.4
    THRESHOLD_WEAK_TREND = 1.7       # 弱趋势/震荡阈值

    # 3.2 RSI 参数 (超买超卖)
    RSI_PERIOD = 14                  # RSI 计算周期
    RSI_OVERBOUGHT = 90              # 超买阈值 (卖出/减仓警告)
    RSI_OVERSOLD = 10                # 超卖阈值 (反弹/买入关注)

    # 3.3 布林带 (Bollinger Bands) 参数
    BB_WINDOW = 16           # 计算周期 (默认20)
    BB_STD = 1.9             # 标准差倍数 (默认2.0)
